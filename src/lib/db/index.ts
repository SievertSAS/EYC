...
class EyCDatabase extends Dexie {
  ...
  constructor() {
    ...
    this.version(3).stores({
      ...
      grupo_resultados:
        "++id, visita_id, grupo_id, equipo_id, completado, sync_status",
    ...
    this.version(4).stores({
      ...
      const oldRows = await (tx as unknown as { table: (name: string) => Dexie.Table }).table("tecnicos").toArray();
      const usuariosTable = tx.table("usuarios");
      for (const row of oldRows) {
        const cargo = row.cargo === "fisico_tecnico" || row.cargo === "ingeniero" || row.cargo === "tecnologo"
          ? "tecnico"
          : row.cargo ?? "tecnico";
